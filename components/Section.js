...
export default function Section(props){
    ...
    const mobileSize = childNodes.length == 1 ? 12 : 12/column
    const tabletSize = childNodes.length == 1 ? 12 : 12/(column+1) 
    const desktopSize = childNodes.length == 1 ? 12 : 12/(column+2) 
    const tvSize = childNodes.length == 1 ? 12 : 12/(column+3) 
    ...
    return(
        <Grid.Container id={sectionID} className={`${css.section} observed`} gap={1} justify="space-evenly" css={{marginBottom:"unset",marginTop:"unset",padding:"5vw",...filling}}> 
            {childNodes.map(child=>
                <>
                    {child.props.hasOwnProperty("grid") ? < Grid justify="center" {...child.props.grid}>{child}</Grid>: <Grid justify="center" {...deviceGrid[showUpTo]}>{child}</Grid> } 
